...
class TestFeaturesFromDataframe:
    ...
    def test_extracts_only_canonical_columns(self, small_track_dataframe: pd.DataFrame) -> None:
        classifier = MusicEmotionClassifier()
...
class TestPredictAndSaveLoad:
    ...
    def test_predict_emotion_returns_label_per_row(
        ...
        classifier.train(feats.values, labels_y, epochs=2, validation_split=0.2, batch_size=8)
    ...
    def test_predict_without_train_raises(self, small_track_dataframe: pd.DataFrame) -> None:
        classifier = MusicEmotionClassifier()
